...
Type Annotation VS Type Inference;
    Type Annotation -> As a developer, we tell the TypeScript compiler about the Type of the variable;
    Type Inference -> TypeScript compiler automatically tries out to figure out the type of a variable; 
    Eg:
        let count: number = 5; // Developer specified the type of count as number;
        let count2 = 6; console.log(typeof count2); // It prints number, b/c TypeScript compiler infers the type of count2 as number(based on initialized value);

How Type Inference works?
    If the type of a varilable is not specified, then
        If it is not initialized, then typescript defaults it to 'any';
            This behaviour can be turned off in tsconfig.json using either noImplicitAny: true OR strict: true;
    If it is having a initial value, type is inferred based on it's initial value;
    
    # --------------------
    let myAny1; // Because there is no initial value, type is inferred as 'Any';
    myAny1 = 10;
    myAny1 = 'test';
    myAny1 = true;

    # --------------------
    let myAny2 = 10; // Because there is a initial value, type is inferred as number;
    # myAny2 = 'test'; // ERROR: Type 'test' is not assignable to type 'number'

    # --------------------
    let myAny3 = myAny2; // As myAny2 is number, type of myAny3 will be number;

    # --------------------
    function add(x: number, y: number): number {
        return x + y;
    }
    let myAny4 = add(2, 3); // As return type of function is number, myAny4 will be number;



As TypeScript compiler can infer the Type, why we should specify the Type explicitly?
    Always use Type Inference, But there are 3 situations where we might need to specify the type explicitly:
        - You declarated a variable, without initializing it in the same statement;
            Example:
                let myAny1; // Because there is no initial value, type is inferred as 'Any';
                myAny1 = 10;        // No Error!
                myAny1 = 'test';    // No Error!
                myAny1 = true;      // No Error!
        - If the return type of a function is any, while assigning the return value to a variable, you may want to specify the type explicitly;
            Example:
                const json = '{"x": 10, "y": 20}';
                const coordinates = JSON.parse(json); // Type of coordinates is inferred as Any, as return type of JSON.parse() is Any;
                console.log(coordinates.asdf); // No Error!
                const coordinates2: {x: number, y: number} = JSON.parse(json); // Explicitly specify the type of coordinates2;
                console.log(coordinates2.asdf); // TSC throws an Error!
        - Sometimes, you want a variable to hold multiple type of values;
            Example: let's create a variable, which can hold number or string;
                let myVar = 10; // Type of myVar is inferred as number;
                myVar = 'hello'; // TSC throws error!
                # Solution 1: Change type of myVar to Any
                let myVar2: any = 10;
                myVar2 = 'hello';
                # Solution 2: Use union Type;
                let myVar3: number | string = 10;
                myVar3 = 'hello';
...
let age: number = 25;
let fullName: string = "Mallikarjuna Rao";
console.log("isDone: " + isDone + ", age: " + age + ", fullName: " + fullName);

// Backquotes are used to escape template strings
let myString: string = `Hello, my name is ${fullName}. I will be ${age + 1} next month.`;
